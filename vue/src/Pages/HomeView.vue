<template>
        ...
        <div class="container mx-auto">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="col-span-3 flex justify-end m-2"> <button
                        ...
                        @click="$router.push('/user/cart')">View Cart</button>
                    ...
                    <div v-for="product in products" :key="product.id" class="bg-white rounded-lg shadow-md p-6">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex flex-row gap-2">
                                <button
                                    class="bg-slate-700 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition duration-300"
    ...
    <div v-if="showCheckout" class="fixed inset-0 flex items-center justify-center w-full bg-black bg-opacity-50 z-50">
        <div class="bg-white rounded-lg p-8 w-1/2 mx-auto shadow-lg">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-2xl font-bold text-gray-900">Product Info</h2>
                <div class="w-6">
                    <CIcon icon="cilX" class="hover:cursor-pointer hover:text-red-500" @click="showCheckout = false" />
                </div>
            </div>
            <div class="flex flex-row gap-4">
                Cart List
...
export default {
    ...
    methods: {
        ...
        addToCart(product) {
            const find = !this.$store.state.cart.find(carts => carts.id == product.id);
            var exceed = false;
            if (!find) {
                var obj = this.$store.state.cart.findIndex(carts => carts.id == product.id);
                var arr = product.quantity <= this.$store.state.cart[obj].quantity;
                if (arr) {
                    exceed = true;
                }
            ...
            if (exceed) {
                ...
                this.$store.commit('setCart', newProduct);
            }
